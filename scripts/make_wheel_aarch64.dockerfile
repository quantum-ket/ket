...
FROM rust:1.82-slim-bullseye AS build_libket_aarch64
...
FROM rust:1.82-slim-bullseye AS build_kbw_aarch64
RUN apt update && apt install -y gcc-aarch64-linux-gnu
RUN rustup target add aarch64-unknown-linux-gnu
...
FROM --platform=linux/arm64 python:3-slim AS package_aarch64
...
RUN python -m auditwheel repair --plat manylinux_2_31_aarch64 dist/ket_lang*.whl
